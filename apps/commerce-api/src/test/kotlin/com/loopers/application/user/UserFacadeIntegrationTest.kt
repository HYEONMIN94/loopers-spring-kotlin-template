...
import com.loopers.application.user.dsl.UserFacadeIntegrationDsl
...
class UserFacadeIntegrationTest @Autowired constructor(
    ...
    inner class SignUp {
        ...
        fun `회원 가입 시 user 정보를 저장한다`() {
            // when
            UserFacadeIntegrationDsl.signUp(userFacade)
        ...
        fun `이미 가입된 ID 로 회원가입 시도 시 실패한다`() {
            // given
            val userInfo = UserFacadeIntegrationDsl.givenSignUp()
